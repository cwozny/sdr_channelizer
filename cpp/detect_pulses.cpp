...
#define NOISE_FLOOR_WINDOW 2048
...
int main(int argc, char *argv[])
{
	...
	* to all wildcard (i.e., "any device") values */
	...
	std::cout << "Noise floor is " << noiseFloor << std::endl;
...
#if defined(DEBUG_OUTPUT)
		std::cout << std::real(iq[n]) << "," << std::imag(iq[n]) << "," << (pulseActive ? 1 : 0) << std::endl;
#endif
	}
