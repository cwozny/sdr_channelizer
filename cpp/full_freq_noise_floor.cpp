...
#define SAMPLE_LENGTH 4194304 // each buffer represents 83.9 milliseconds
#define INT16_BUFFER_LENGTH 8388608
...
int main(int argc, char *argv[])
{
	...
	* to all wildcard (i.e., "any device") values */
	...
	int16_t* rx_samples = new int16_t[INT16_BUFFER_LENGTH];
	...
	for(bladerf_frequency freq = 70e6; freq <= 5999e6; freq += 1e6)
	{
		iq.clear();
		mag.clear();
		memset(rx_samples, 0, INT16_BUFFER_LENGTH*sizeof(int16_t));
...
//		std::cout << "Asked for " << freq * 1e-6 << " MHz, got " << actualFreq * 1e-6 << " MHz" << std::endl;
		...
		else
		{
			...
			std::cout << freq*1e-6 << "," << noiseFloor << std::endl;
		}
	}
